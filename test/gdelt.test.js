...
test('buildUrl produces a valid GDELT DOC query with %20-encoded phrases', () => {
  const url = gdelt.buildUrl({ timespan: '3d', maxrecords: 50 });
  assert.ok(url.startsWith('https://api.gdeltproject.org/api/v2/doc/doc?'));
  assert.ok(url.includes('mode=ArtList') && url.includes('format=json'));
  assert.ok(decodeURIComponent(url).includes('"data center"'));
});

test('articlesToLeads drops residential and keeps source-traceable leads', () => {
  const leads = gdelt.articlesToLeads(FIXTURE.articles);
  assert.strictEqual(leads.length, 3); // residential Lane Homes dropped
  for (const l of leads) {
    assert.strictEqual(l.sample, false);
    assert.strictEqual(l.needsReview, true);
    assert.strictEqual(l.source, 'gdelt');
    assert.ok(l.signals[0].sourceUrl && l.signals[0].date);
  }
  const fund = leads.find((l) => l.name.startsWith('Sahel'));
