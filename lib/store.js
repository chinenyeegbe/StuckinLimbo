...
const { dedupContacts, bestContact } = require('./contacts');
...
async function loadLeads() {
  let stored = null;
  try {
    stored = await kv.get(LEADS_KEY);
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error('[store] read failed, serving seed:', err.message);
  }
  if (stored && Array.isArray(stored.leads)) return stored.leads;
  // First run (or read failure): generate the illustrative seed. Persistence is
  // best-effort — the dashboard must never be empty just because the store is
  // unwritable, so a save failure here is logged, not thrown.
  const leads = mergeDuplicates(generateSeedLeads());
  try {
    await saveLeads(leads);
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error('[store] seed persist failed (continuing in-memory):', err.message);
  }
  return leads;
}
...
function mergeDuplicates(leads) {
  ...
  for (const lead of leads) {
    ...
    if (!byKey.has(key)) {
      const contacts = dedupContacts(lead.contacts || []);
      byKey.set(key, {
        ...lead,
        signals: dedupSignals(lead.signals || []),
        contacts,
        primaryContact: lead.primaryContact || bestContact(contacts),
      });
    } else {
      ...
      // Reach-out methods accumulate across every signal for one human.
      existing.contacts = dedupContacts([...(existing.contacts || []), ...(lead.contacts || [])]);
      existing.primaryContact = bestContact(existing.contacts) || existing.primaryContact;
      existing.enriched = existing.enriched || lead.enriched || (existing.contacts.length > 0);
    }
  }
