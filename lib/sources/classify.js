...
const { deriveContacts, bestContact } = require('../contacts');
...
// Market inference — map a mentioned city/country in free text to a display
// market so social posts (which carry no geo) still score and filter on place.
const MARKET_TERMS = [
  ['Lagos', 'Lagos, Nigeria'], ['Abuja', 'Abuja, Nigeria'], ['Nigeria', 'Nigeria'],
  ['Nairobi', 'Nairobi, Kenya'], ['Mombasa', 'Mombasa, Kenya'], ['Kenya', 'Kenya'],
  ['Accra', 'Accra, Ghana'], ['Ghana', 'Ghana'], ['Cairo', 'Cairo, Egypt'], ['Egypt', 'Egypt'],
  ['Johannesburg', 'Johannesburg, South Africa'], ['Cape Town', 'Cape Town, South Africa'],
  ['South Africa', 'South Africa'], ['Kigali', 'Kigali, Rwanda'], ['Rwanda', 'Rwanda'],
  ['Addis Ababa', 'Addis Ababa, Ethiopia'], ['Ethiopia', 'Ethiopia'],
  ['Dar es Salaam', 'Dar es Salaam, Tanzania'], ['Tanzania', 'Tanzania'],
  ['Kampala', 'Kampala, Uganda'], ['Uganda', 'Uganda'], ['Dakar', 'Dakar, Senegal'],
  ['Senegal', 'Senegal'], ['Casablanca', 'Casablanca, Morocco'], ['Morocco', 'Morocco'],
  ['Abidjan', "Abidjan, Côte d'Ivoire"], ['Lusaka', 'Lusaka, Zambia'], ['Zambia', 'Zambia'],
  ['Maputo', 'Maputo, Mozambique'], ['Mozambique', 'Mozambique'],
  ['Bengaluru', 'Bengaluru, India'], ['Bangalore', 'Bengaluru, India'], ['Mumbai', 'Mumbai, India'],
  ['Delhi', 'Delhi, India'], ['Hyderabad', 'Hyderabad, India'], ['Chennai', 'Chennai, India'],
  ['India', 'India'], ['Jakarta', 'Jakarta, Indonesia'], ['Indonesia', 'Indonesia'],
  ['Ho Chi Minh', 'Ho Chi Minh City, Vietnam'], ['Hanoi', 'Hanoi, Vietnam'], ['Vietnam', 'Vietnam'],
  ['Manila', 'Manila, Philippines'], ['Philippines', 'Philippines'], ['Dhaka', 'Dhaka, Bangladesh'],
  ['Bangladesh', 'Bangladesh'], ['Karachi', 'Karachi, Pakistan'], ['Lahore', 'Lahore, Pakistan'],
  ['Pakistan', 'Pakistan'], ['São Paulo', 'São Paulo, Brazil'], ['Sao Paulo', 'São Paulo, Brazil'],
  ['Brazil', 'Brazil'], ['Bogotá', 'Bogotá, Colombia'], ['Bogota', 'Bogotá, Colombia'],
  ['Colombia', 'Colombia'], ['Mexico City', 'Mexico City, Mexico'], ['Mexico', 'Mexico'],
];

function detectMarket(text) {
  const t = text || '';
  for (const [needle, label] of MARKET_TERMS) {
...
function buildLead(item) {
  ...
  // Prefer the source's own geo; otherwise infer one from the text so social
  // posts still carry a market for scoring, filtering, and display.
  const market = item.market || detectMarket(text);
  ...
  // Every lead ships with concrete, public reach-out methods so the founder can
  // act without a separate enrichment step.
  const contacts = deriveContacts(item, { org, name });
  const primaryContact = bestContact(contacts);
