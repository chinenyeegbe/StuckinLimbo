...
function present(lead, scored, now = Date.now()) {
  return {
    ...
    contactable: !!(lead.contacts && lead.contacts.length),
    score: scored.score,
...
async function getLeads(query) {
  ...
  if (query.contactableOnly === '1') items = items.filter((l) => l.contactable);
  // Reachable leads break ties — a high score you can act on beats one you can't.
  if (sort === 'score') {
    items.sort((a, b) => b.score - a.score || (b.contactable - a.contactable));
...
async function getDigest(query) {
  ...
  const scored = scoreAll(leads, now)
    ...
    .sort((a, b) => b.score - a.score || (b.contactable - a.contactable));
